...
pub(crate) fn compose_fujifilm_classic(
    ...
    let wordmark_pt = ((frame.font_size as f32 * spec.primary_font_scale * 1.7).max(20.0)) * rs;
    ...
    rend.draw(&mut canvas, "FUJIFILM", placed_x as f32, caption_top, wordmark_pt, true, FUJI_GREEN);

    // Film simulation pill (use first custom line or default)
    let sim_text = {
    ...
    let pill_pt = detail_pt.max(10.0 * rs);
    ...
    let pill_y = caption_top + wordmark_pt * 0.85 - pill_h * 0.78;
    ...
    if !detail.is_empty() {
        let detail_y = caption_top + wordmark_pt * 0.85 + (detail_pt * 1.6).max(20.0 * rs);
        rend.draw(&mut canvas, &detail, placed_x as f32, detail_y, detail_pt, false, FUJI_MUTED);
    }

    // Right: params + date
    let right_x = placed_x + pw;
    let params_pt = (frame.font_size as f32 * spec.primary_font_scale * 1.2).max(13.0 * rs);
    ...
    let params_baseline = caption_top + wordmark_pt * 0.85;
    if !params.is_empty() {
        let pw_meas = rend.measure(&params, params_pt, true);
        rend.draw(&mut canvas, &params, right_x as f32 - pw_meas, params_baseline - params_pt + rend.ascent(params_pt, true), params_pt, true, FUJI_INK);
    }
    if !date.is_empty() {
        let dw = rend.measure(&date, detail_pt, false);
        let dy = params_baseline + (detail_pt * 1.6).max(20.0 * rs);
        rend.draw(&mut canvas, &date, right_x as f32 - dw, dy, detail_pt, false, FUJI_MUTED);
    }
